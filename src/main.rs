...
fn main() {
    ...
    match read_and_parse(filename) {
        Ok(nodes) => {
            ...
            let unique_nodes = unique_nodes(&sample); // calculate unique nodes

            println!("Calculating average distance..."); // average distance
            let average_distance = average_distance(&sample, &nodes);
            ...
            println!("Number of unique nodes in sample: {}", unique_nodes);
